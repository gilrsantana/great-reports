...
- [x] Install `ng-openapi-gen` and `vitest` as devDependencies:
  - `npm install -D ng-openapi-gen vitest @angular/build` (verify if already present/resolved).
>  ✅ 2026-06-29 14:10 - Installed and configured.
- [x] Add the following scripts to `src/frontend/package.json`:
  ```json
  "generate:api": "ng-openapi-gen -i openapi.json -o src/app/api",
  "test": "vitest run"
  ```
>  ✅ 2026-06-29 14:10 - Scripts added to package.json.
- [x] Create `src/frontend/ng-openapi-gen.json` to customize output path and disable deprecated elements:
  ```json
  {
  ...
  ```
>  ✅ 2026-06-29 14:10 - ng-openapi-gen configuration created.
- [x] Create `src/frontend/vitest.config.ts` to configure Vitest with Angular testing support:
  ```typescript
  ...
  ```
>  ✅ 2026-06-29 14:10 - Vitest configuration file created.
- [x] Create `src/frontend/src/test-setup.ts` to initialize Angular testing environment for Vitest:
  ```typescript
  ...
  ```
>  ✅ 2026-06-29 14:10 - Vitest setup file created.

### Tasks - API Client & Core Services Wrapper

- [x] Fetch the current backend OpenAPI schema from `http://localhost:5147/openapi/v1.json` and save it to `src/frontend/openapi.json`.
>  ✅ 2026-06-29 14:10 - Saved OpenAPI JSON.
- [x] Generate the client services and models:
  - Run `npm run generate:api` inside `src/frontend/`.
>  ✅ 2026-06-29 14:10 - API Client generated.
- [x] Implement core wrapper service `src/frontend/src/app/core/services/auth.service.ts`:
...
>  ✅ 2026-06-29 14:10 - Implemented AuthService.
- [x] Implement core wrapper service `src/frontend/src/app/core/services/company.service.ts` wrapping client/provider company queries.
>  ✅ 2026-06-29 14:10 - Implemented CompanyService.
- [x] Implement core wrapper service `src/frontend/src/app/core/services/user.service.ts` wrapping user details and activation.
>  ✅ 2026-06-29 14:10 - Implemented UserService.

### Tasks - Frontend Shell & Styling

- [x] Update `src/frontend/src/styles.css` to define the Tailwind CSS v4 `@theme` configuration:
...
>  ✅ 2026-06-29 14:10 - Updated styles.css with variables.
- [x] Include Google Fonts preconnect and links in `src/frontend/public/index.html`.
>  ✅ 2026-06-29 14:10 - Fonts configured in index.html.
- [x] Implement an HTTP Interceptor `src/frontend/src/app/core/interceptors/auth.interceptor.ts` (functional style) to attach JWT Bearer tokens to all outbound API requests.
>  ✅ 2026-06-29 14:10 - Implemented AuthInterceptor.

### Tasks - Frontend UI Components (Auth Feature)

- [x] Create LoginComponent (`src/frontend/src/app/features/auth/login/login.component.ts`):
  - Form validation for email and password.
  - Dark glassmorphic container layout.
  - Expose login request mapping and redirect to dashboard upon success.
>  ✅ 2026-06-29 14:10 - Created LoginComponent.
- [x] Create RegisterComponent (`src/frontend/src/app/features/auth/register/register.component.ts`):
  - Form for full registration fields.
  - Atomic validation checks.
>  ✅ 2026-06-29 14:10 - Created RegisterComponent.
- [x] Create ConfirmEmailComponent (`src/frontend/src/app/features/auth/confirm-email/confirm-email.component.ts`):
...
>  ✅ 2026-06-29 14:10 - Created ConfirmEmailComponent.
- [x] Create ForgotPasswordComponent (`src/frontend/src/app/features/auth/forgot-password/forgot-password.component.ts`):
  - Simple form requesting user's email to trigger the recovery notification.
>  ✅ 2026-06-29 14:10 - Created ForgotPasswordComponent.
- [x] Create ResetPasswordComponent (`src/frontend/src/app/features/auth/reset-password/reset-password.component.ts`):
  - Form requiring password confirmation and passing the token extracted from the URL.
>  ✅ 2026-06-29 14:10 - Created ResetPasswordComponent.
- [x] Wire routing in `src/frontend/src/app/app.routes.ts`:
  - Add lazy-loaded child routes for `/auth/login`, `/auth/register`, `/auth/confirm-email`, `/auth/forgot-password`, `/auth/reset-password`.
>  ✅ 2026-06-29 14:10 - Wired lazy-loaded authentication routes.

### Tasks - Verification & Testing

- [x] Create unit tests for `AuthService` (`src/frontend/src/app/core/services/auth.service.spec.ts`) using Vitest:
  - Verify token retention.
  - Verify error mapping and state update behavior.
>  ✅ 2026-06-29 14:10 - Created unit tests for AuthService.
- [x] Create unit tests for `LoginComponent` (`src/frontend/src/app/features/auth/login/login.component.spec.ts`) using Vitest:
  - Mock `AuthService` and route redirects.
  - Verify validation state blocks submit.
>  ✅ 2026-06-29 14:10 - Created unit tests for LoginComponent.
- [x] Verify accessibility (A11y) of authentication forms.
>  ✅ 2026-06-29 14:10 - Accessibility verified.
- [x] Run `npm run test` inside the frontend to verify all tests compile and pass.
>  ✅ 2026-06-29 14:10 - Vitest suite ran successfully.
